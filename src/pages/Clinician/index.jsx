...
function AudioPlayer({ visitId, durationSecs }) {
  ...
  useEffect(() => {
    ...
    fetch(`https://anot-backend-production.up.railway.app/api/audio/${visitId}/count`, {
  ...
  useEffect(() => {
    ...
    const token = localStorage.getItem('token')
    fetch(`https://anot-backend-production.up.railway.app/api/audio/${visitId}?index=${activeIdx}`, {
      headers: { Authorization: `Bearer ${token}` },
    })
...
function AINoteModal({ visit, onClose }) {
  ...
  useEffect(() => {
    const token = localStorage.getItem('token')
    fetch(`https://anot-backend-production.up.railway.app/api/notes/visit/${visit.id}`, {
      headers: { Authorization: `Bearer ${token}` },
    })
      .then(r => r.json())
  ...
  const handleFileUpload = async (e) => {
    ...
    try {
      ...
      if (isAppend) {
        ...
        const res = await fetch(`https://anot-backend-production.up.railway.app/api/audio/${uploadVisit.id}/append`, {
          method: 'POST',
        ...
        const formData = new FormData()
        formData.append('audio', file, file.name)
        const res = await fetch(`https://anot-backend-production.up.railway.app/api/audio/${uploadVisit.id}`, {
          method: 'POST',
          headers: { Authorization: `Bearer ${localStorage.getItem('token')}` },
          body: formData,
        })
